...
import { floralProducts, floralStudio } from '../data/floralWorks'
...
const allProducts = [...floralProducts, ...florajetProducts]
...
export default function FlowersDetail() {
  ...
  const isGreenfield = !detail?.source // 无独立 source 字段的为 Greenfield 商品
  ...
  if (!detail) {
    return (
      <div className="cd-page">
        ...
        <div className="cd-loading"><p>未找到该花艺作品</p></div>
      </div>
    )
  }
    ...
    <div className="cd-page">
      ...
      <section className="cd-hero">
        ...
        <div className="cd-hero__content">
          <span className="cd-hero__badge">{detail.categoryCn} · {isGreenfield ? floralStudio.name : detail.source!.name.replace('（测试数据）', '')}</span>
      ...
      <div className="cd-content">
        ...
        <section className="cd-block">
          <h2 className="cd-block__title">{isGreenfield ? '花艺工作室' : '出品方'}</h2>
          <div className="cd-about__body">
            {isGreenfield ? (
              <>
                <p>{floralStudio.introCn}</p>
                <p>📍 {floralStudio.location} · 👩‍🎨 创始人：{floralStudio.founder}</p>
              </>
            ) : (
              <>
                <p>Florajet 是法国知名鲜花速递品牌，由花艺师手工包扎，法国全境配送，最快次日送达。本商品来自其生日专题精选（测试数据）。</p>
                <p>📍 法国全境配送 · 💐 花艺师手工包扎</p>
              </>
            )}
          </div>
        ...
        <section className="cd-source">
          {isGreenfield ? (
            <p>数据来源：<a href={floralStudio.sourceUrl} target="_blank" rel="noreferrer">{floralStudio.sourceUrl}</a>（测试数据）</p>
          ) : (
